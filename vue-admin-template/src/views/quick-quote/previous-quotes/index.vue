    ...
    <div class="content">
      ...
      <el-table
        ...
        >
          <template slot-scope="scope">
            <el-button
              @click.native.prevent="restartProject(scope.$index, scope.row)"
              type="text">
              <i class="el-icon-refresh-left" style="font-size: 22px;"></i>
            </el-button>
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      restartProject(index,data){
        return this.$confirm('此操作将重新启动项目, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
